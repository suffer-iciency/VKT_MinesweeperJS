...
export function updateStatusBarRemainingMines(mineField)
{
    ...
    minesRemaining = minesRemaining.toString().padStart(3, '0');
    minesRemaining = minesRemaining.split('').map((item) => {
      return "<img src='" + MenuNumbers[item] + "'>"
    }).join('')

  document.getElementById(Ids.gameScreen.statusBar.minesRemainingLabel).innerHTML = `${minesRemaining}`;
}
...
export function uncoverArea(mineField, x, y) {
  if (mineField.grid[y][x].state === TileState.COVERED
      ...
      || mineField.grid[y][x].state === TileState.QUESTION_MARK
  )
  {
    return;
  }
...
export function toggleFlagTile(mineField, x, y)
{
  ...
  else if (mineField.grid[y][x].state === TileState.QUESTION_MARK) {
...
export function gameOver(mineField)
{
  ...
  let timerSeconds = Math.floor(timeDifferenceInSeconds % 999);
    ...
    for (let x = 0; x < mineField.width; x++) {
      if (mineField.grid[y][x].state === TileState.FLAGGED && !mineField.grid[y][x].isMine) {
        ...
        if (isGridClearedTemp) {
          ...
          mineField.grid[y][x].isMine = TileState.HIT_MINE;
          mineField.grid[y][x].state = TileState.MINE
        }
      }
    }
